...
// AI types still referenced by client.ts's remaining (mobile/memory) code.
...
// references project/semantic types that remain here).
import type { AtlasCapture, CaptureTriageAction } from './captures'
// Entity/telemetry types still referenced by client.ts's kept code: AtlasCheckin['state'] by the
// task-agenda context, and the behavior types by the kept Bitacula normalization/analysis surface.
import type {
  AtlasCheckin,
  AtlasBehavior,
  AtlasBehaviorLog,
